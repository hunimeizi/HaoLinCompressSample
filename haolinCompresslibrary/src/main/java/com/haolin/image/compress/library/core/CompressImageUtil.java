...
public class CompressImageUtil {
    ...
    private File getPhotoCacheDir(File file) {
        String path =  new CompressPathUtils(context).getPicturesDirPathCompress();
        if (TextUtils.isEmpty(config.getCacheDir())) config.setCacheDir(path);
        File mCacheDir = new File(config.getCacheDir());
        Log.e("lyb >>> ", mCacheDir.getAbsolutePath());
